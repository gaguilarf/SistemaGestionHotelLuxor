...
const EditGroup = () => {
  ...
  useEffect(() => {
    if (group) {
      setFormData({
        name: group.name || '',
        permissions: group.permissions && group.permissions.map ? group.permissions.map(p => p.id || p) || [] : []
      });
    }
  }, [group]);
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-gray-600 font-medium text-sm md:text-base">Cargando grupo...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 md:p-6 m-4 md:m-0">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            ...
            <p className="text-yellow-600 text-xs md:text-sm">El grupo solicitado no existe.</p>
          </div>
        </div>
        <Button variant="outline" onClick={handleCancel} className="mt-4 w-full md:w-auto">
          Volver a grupos
    ...
    <div className="space-y-4 md:space-y-6 p-4 md:p-0">
      ...
      <div className="flex flex-col space-y-3 md:flex-row md:items-center md:space-y-0 md:space-x-4">
        ...
        <div className="text-center md:text-left">
          <h1 className="text-xl md:text-2xl font-bold text-gray-900">Editar Rol</h1>
          <p className="text-gray-600 text-sm">Modifica la información de {group.name}</p>
        </div>
      </div>
